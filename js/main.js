...
function createVis(data) {
    ...
    let prioVis = new Example_component("priovis", allData, metaData);
    ...
    eventHandler.bind("selectionChanged", function(event) {
        countVis.onSelectionChange(event.detail[0], event.detail[1]);
        ageVis.onSelectionChange(event.detail[0], event.detail[1]);
        prioVis.onSelectionChange(event.detail[0], event.detail[1]);
    });
}
